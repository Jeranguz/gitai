import litellm
from litellm.exceptions import APIConnectionError, AuthenticationError, InternalServerError
...
def _call_llm(prompt: str, config: dict) -> str:
    ...
    model_string = f"{provider}/{config['model']}"
    ...
    try:
        ...
        raise SystemExit(
            f"[gitai] The {provider} API is temporarily overloaded. Please try again in a moment."
        )
